...
\begin{tikzpicture}[thick, node distance=2.5cm,auto,>=latex', scale=1, transform shape]
    ...
    \node [int] (c) [right of=a,node distance=2cm,align=center] {$\displaystyle\min_{\mathbf{\mathbf{x,u}}} J(\mathbf{u},\mathbf{x},\mathbf{x}(k),\mathbf{d})$ \\ $\text{s.t.} \quad \mathbf{Fz} = \mathbf{c}$ \\ \hspace{3.3em} $\underline{\mathbf{z}} \leq \mathbf{z} \leq \overline{\mathbf{z}}$ };
    ...
    \node (m) [above of=f, node distance=2.2cm] {$\mathbf{d}$};
